...
SHRT_Val_Format = ['T3H', 'REH', 'UUU', 'VVV', 'WSD', 'VEC', 'PTY', 'SKY', 'R06', 'POP', 'TMX', 'TMN', 'SKY', 'S06', 'WAV']
SHRT_Val_Null = {'T3H':'-50', 'REH':'-1', 'UUU':'-100', 'VVV':'-100', 'WSD':'-1', 'VEC':'-1', 'PTY':'-1', 'SKY':'-1', 
                 'R06':'-1', 'POP':'-1', 'TMX':'-50', 'TMN':'-50', 'SKY':'-50', 'S06':'-50', 'WAV':'-50'}
SHRT_Predict_Hour_Format = ['4', '7', '10', '13', '16', '19', '22', '25', '28', '31', '34', '37', '40', '43', '46', '49', 
                            '52', '55', '58', '61', '64', '67']
SHRT_Predict_Hour_Size = {'02':15, '05':22, '08':21, '11':20, '14':19, '17':18, '20':17, '23':16}
SHRT_Size = 37697
...
def OptionSetter():
    ...
    X Range : 0 ~ 148
    Y Range : 0 ~ 252
    Value List : {0}""".format(', '.join(SHRT_Val_Format))
...
def InitSetter(Year, Month, Day, IsOld, IsNew):
    ...
    return PathParserConfig.get('PATH', 'SHRT').replace("%YEAR",Year).replace("%MONTH", Month).replace("%DAY", Day)
...
def GetHourList(StartTime, EndTime):
    # 시간 리스트 생성
    TimeList = list()
    RunHour = StartTime.hour
    TempDelta = 2 - (RunHour % 3)
    RunTime = StartTime + timedelta(hours = TempDelta)
    EndTim2 = EndTime + timedelta(hours = TempDelta)
    while RunTime <= EndTim2:
...
def ArgCheck(option):
    ...
    try:
        ...
        if (Value not in SHRT_Val_Format):
...
def MakeGribParseOption(Year, Month, Day, Hour, Var, Path, output, PHour, IsOld, IsNew):
    ...
    StrOutput = output
    if IsOld:
        StrInput = Path + "/DFS_SHRT_GRD_GRB1_{0}.{1}{2}{3}{4}00".format(Var, Year, Month, Day, Hour)
        StrFormat = "SHRT_OLD"
    elif IsNew:
        StrInput = Path + "/DFS_SHRT_GRD_GRB3_{0}.{1}{2}{3}{4}00".format(Var, Year, Month, Day, Hour)
        StrFormat = "SHRT"
    else:
        StrInput = Path + "/DFS_SHRT_GRD_GRIB_{0}.{1}{2}{3}{4}00".format(Var, Year, Month, Day, Hour)
        StrFormat = "SHRT"
...
if __name__ == '__main__':
    ...
    for DateTimeH in TimeList:
        # 시간에 따라 다른 파일이 생성
        Output_Base = "SHRT_" + Value + "_" + DateTimeH + ".csv"
        ...
        IsOld = IsOldSHRT(UDateTime)
        IsNew = IsNewSHRT(UDateTime)
        SHRT_Predict_Hour_FormatH = SHRT_Predict_Hour_Format[0:SHRT_Predict_Hour_Size[StrHour]]

        HEAD = '\n'+DateTimeH + ',' + ','.join( x + 'hr' for x in SHRT_Predict_Hour_FormatH) + '\n'
        ...
        with open(Output_tmp, 'w') as OutFP:
            ...
            for PHour in SHRT_Predict_Hour_FormatH:
                # 예보 시간에 따른 Grib 반복작업 
                DateTimeP = DateTimeH + "_{0}hr".format(PHour)
#                OutputH_tmp_Var = OutputH_tmp.replace('SHRT_', 'SHRT_{0}_'.format(Value))
        ...
        cat_head = "cat {0}/head_shrt.txt >> {1}".format(PROG_DIR, Output)
